...
public class ApiShop : IShopApi
{
    ...
    public bool? IsClientHasItem(CCSPlayerController player, int itemID)
    {
        if (player == null)
            return null;
        ...
        return _shop.playerInfo[player.Slot].ItemList.Find(x => x.item_id == itemID) != null;
    }
    
    public bool UseClientItem(CCSPlayerController player, int itemID)
    {
        if (player == null)
            return false;
        ...
        var Itemlist = _shop.ItemsList.Find(x => x.ItemID == itemID);
        var PlayerList = _shop.playerInfo[player.Slot].ItemList.Find(x => x.item_id == itemID);
        if (Itemlist == null || PlayerList == null)
            return false;

        // use bool result in future
        _shop.OnChooseAction(player, Itemlist.ItemName, Itemlist.UniqueName, itemID, Itemlist.Category, PlayerList, false);
