...
public class Shop : BasePlugin, IPluginConfig<ShopConfig>
{
    ...
    public override void OnAllPluginsLoaded(bool hotReload)
    {
        ...
        catch (KeyNotFoundException ex) { _ = ex; } // Убираем ошибку The given key 'menu:nfcore' was not present in the dictionary. если на сервере нету плагина
    }
    ...
    {
        var menu = CreateMenu(Localizer["Menu_AdminTitle"]);
        menu.AddMenuOption(Localizer["Menu_AdminAddCredits"], (player, _) => OpenAdminCreditsPlayersMenu(player, "add"));
        menu.AddMenuOption(Localizer["Menu_AdminSetCredits"], (player, _) => OpenAdminCreditsPlayersMenu(player, "set"));
        menu.AddMenuOption(Localizer["Menu_AdminTakeCredits"], (player, _) => OpenAdminCreditsPlayersMenu(player, "take"));
        menu.AddMenuOption(Localizer["Menu_AdminAddItem"], (player, _) => OpenAdminItemPlayerMenu(player, "add"));
        menu.AddMenuOption(Localizer["Menu_AdminTakeItem"], (player, _) => OpenAdminItemPlayerMenu(player, "take"));
        menu.Open(player);
    }

    public void OpenAdminCreditsPlayersMenu(CCSPlayerController player, string type)
    {
        var menu = CreateMenu(Localizer["Menu_AdminChoosePlayers"]);
        foreach (var players in Utilities.GetPlayers().Where(x => !x.IsBot && !x.IsHLTV))
        {
            menu.AddMenuOption($"{players.PlayerName} [{GetClientCredits(players)}]", (player, _) =>
            {
                ...
                OpenAdminCreditsMenu(player, players, type);
            });
        }
    ...
    public void OpenAdminItemPlayerMenu(CCSPlayerController player, string type)
    {
        var menu = CreateMenu(Localizer["Menu_AdminChoosePlayersTitle"]);
        foreach (var players in Utilities.GetPlayers().Where(x => !x.IsBot && !x.IsHLTV))
        {
            menu.AddMenuOption($"{players.PlayerName} [{GetClientCredits(players)}]", (player, _) =>
            {
                ...
                OpenAdminItemMenu(player, players, type);
            });
        }
    ...
    public void OpenAdminCreditsMenu(CCSPlayerController player, CCSPlayerController target, string type)
    {
        IMenu menu;
        switch (type)
        {
            case "add":
            {
                menu = CreateMenu(Localizer["Menu_AdminAddCreditsTitle"]);
                break;
            }
            case "set":
            {
                menu = CreateMenu(Localizer["Menu_AdminSetCreditsTitle"]);
                break;
            }
            case "take":
            {
                menu = CreateMenu(Localizer["Menu_AdminTakeCreditsTitle"]);
                break;
            }
            default:
                return;
        ...
        if (type == "set") menu.AddMenuOption("0", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("100", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("1000", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("10000", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("100000", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("1000000", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.AddMenuOption("10000000", (player, option) => CompleteActionCredits(player, target, type, Convert.ToInt32(option.Text)));
        menu.PostSelectAction = PostSelectAction.Reset;

        menu.Open(player);
    }

    public void CompleteActionCredits(CCSPlayerController player, CCSPlayerController target, string type, int credits)
    {
        if (!target.IsValid || playerInfo[target.Slot] == null)
        {
            player.PrintToChat(StringExtensions.ReplaceColorTags(Localizer["PlayerNotAvailable"]));
            OpenAdminMenu(player);
            return;
        ...
        switch (type)
        {
            case "add":
                {
                    AddClientCredits(target, credits, IShopApi.WhoChangeCredits.ByAdminCommand);
                    Server.PrintToChatAll(StringExtensions.ReplaceColorTags(Localizer["Command_AddCredits", player == null ? "Console" : player.PlayerName, credits, target.PlayerName]));
                    break;
                }
            case "set":
                {
                    SetClientCredits(target, credits, IShopApi.WhoChangeCredits.ByAdminCommand);
                    Server.PrintToChatAll(StringExtensions.ReplaceColorTags(Localizer["Command_SetCredits", player == null ? "Console" : player.PlayerName, credits, target.PlayerName]));
                    break;
                }
            case "take":
                {
                    TakeClientCredits(target, Convert.ToInt32(credits), IShopApi.WhoChangeCredits.ByAdminCommand);
                    Server.PrintToChatAll(StringExtensions.ReplaceColorTags(Localizer["Command_TakeCredits", player == null ? "Console" : player.PlayerName, credits, target.PlayerName]));
                    break;
                }
        }
    }
    public void OpenAdminItemMenu(CCSPlayerController player, CCSPlayerController target, string type)
    {
        IMenu menu;
        switch (type)
        {
            case "add":
                {
                    menu = CreateMenu(Localizer["Menu_AdminAddChooseCategoryTitle"]);
                    break;
                }
            case "take":
                {
                    menu = CreateMenu(Localizer["Menu_AdminTakeChooseCategoryTitle"]);
                    break;
                }
            default:
                return;
        ...
        foreach (var category in CategoryList)
        {
            menu.AddMenuOption(category.Value, (player, _) =>
            {
                if (type == "add")
                {
                    var menu = CreateMenu(Localizer["Menu_AdminChooseAddItem"]);
                    ...
                    foreach (var item in ItemsList)
                    {
                        if (item.Category == category.Key)
                        {
                            ...
                            menu.AddMenuOption(translate, (player, option) =>
                            {
                                if (!target.IsValid || playerInfo[target.Slot] == null)
                                {
                                    player.PrintToChat(StringExtensions.ReplaceColorTags(Localizer["PlayerNotAvailable"]));
                                ...
                                GivePlayerItem(target, item, 1);
                                if (!IsItemFinite(item)) option.Disabled = true;
                                Server.PrintToChatAll(StringExtensions.ReplaceColorTags(Localizer["Command_AddItem", player == null ? "Console" : player.PlayerName, translate, target.PlayerName]));
                            });
                        }
                    }

                    // В будущем заменить подсчет кол-во предметов на такой вариант
                    if (menu.MenuOptions.Count == 0) menu.AddMenuOption(Localizer["Menu_AdminItemsNotFound"], null!, true);
                ...
                {
                    var menu = CreateMenu(Localizer["Menu_AdminChooseTakeItem"]);

                    foreach (var il in playerInfo[player.Slot].ItemList)
                    {
                        var info = ItemsList.Find(x => x.ItemID == il.item_id);
                        if (info != null && info.Category == category.Key)
                        {
                            string translate = info.ItemName + $"{(IsItemFinite(info) ? $" ({il.count})" : "")}";
                            menu.AddMenuOption(translate, (player, option) =>
                            {
                                if (!target.IsValid || playerInfo[target.Slot] == null)
                                {
                                ...
                                TakePlayerItem(target, info, 1);
                                if (IsItemFinite(info) && il.count - 1 > 0) option.Text = "";
                                else option.Disabled = true;
                                Server.PrintToChatAll(StringExtensions.ReplaceColorTags(Localizer["Command_TakeItem", player == null ? "Console" : player.PlayerName, translate, target.PlayerName]));
                            });
                        }
                    }
    ...
    public void OpenShopMenu(CCSPlayerController? player)
    {
        ...
        if (AdminManager.PlayerHasPermissions(player, Config.AdminFlag))
            menu.AddMenuOption(Localizer["Menu_Admin"], (player, _) => OpenAdminMenu(player));
        menu.Open(player);
    }
    ...
    public void OpenFunctionMenu(CCSPlayerController player)
    {
        ...
        foreach(var functions in _api!.FunctionsCallback)
        {
            menu.AddMenuOption(Localizer[functions.Display], (players, _) => {
                CounterStrikeSharp.API.Modules.Menu.MenuManager.CloseActiveMenu(players);
            ...
            try
            {
                await using (var connection = new MySqlConnection(dbConnectionString))
                {
                    ...
                    else
                    {
                        ...
                        {
                            Count = playerInfo[player.Slot].ItemList[index].count + 1,
                            playerID = playerInfo[player.Slot].DatabaseID,
                            itemID = ItemID
                        });
                    }
    ...
    public bool OnChooseAction(CCSPlayerController player, string ItemName, string UniqueName, int ItemID, string Category, ItemInfo playerList, bool displayMenu = true)
    {
        // state item
                ...
                await using (var connection = new MySqlConnection(dbConnectionString))
                {
                    ...
                    else
                    {
                        if (new_count > 0)
                        {
                            ...
                            playerInfo[player.Slot].ItemList[index].count = new_count;
                        }
                        else
                        {
                            await connection.ExecuteAsync("DELETE FROM `shop_boughts` WHERE `player_id` = @playerID AND `item_id` = @itemID", new
    ...
    public async Task<int> AddItemInDB(string Category, string UniqueName, string ItemName, int BuyPrice, int SellPrice, int Duration, int Count)
    {
        ...
        // Я НА ЭТО БЛЯТЬ ПОЛДНЯ УБИЛ ЧТОБЫ ПОФИКСИТЬ ДОБАВЛЕНИЕ ПРЕДМЕТОВ!!!! <okak>
        Server.NextFrame(() =>
        {
